...
| Mobile Flutter shell or bridge lifecycle | `mobile/gui_shell/README.md`, `docs/agent/architecture-map.md`, relevant OpenSpec change | `cd mobile/gui_shell && flutter analyze && flutter test` | Android/iOS lifecycle assumptions changed, secure storage changed, or host-bridge semantics changed |
| Build scripts, packaging, or CI workflows | `docs/build-workflows.md`, relevant OpenSpec change, `Makefile` | `./scripts/build-go-matrix.sh windows/amd64`; `make smoke-android-embedded-host`; `go test ./...`; `go build ./...` | Windows GUI packaging, toolchain pinning, or CI workflow wiring changed |
