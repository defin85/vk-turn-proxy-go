...
| Build scripts, packaging, or CI workflows | `docs/build-workflows.md`, `openspec/specs/native-build-workflows/spec.md`, `Makefile` | `./scripts/build-go-matrix.sh windows/amd64`; `make build-gui-linux`; `make smoke-android-embedded-host`; `go test ./...`; `go build ./...` | desktop packaging, toolchain pinning, or CI workflow wiring changed |
