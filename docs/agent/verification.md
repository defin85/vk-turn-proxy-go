...
| Build scripts, packaging, or CI workflows | `docs/build-workflows.md`, `openspec/specs/native-build-workflows/spec.md`, `Makefile` | `./scripts/build-go-matrix.sh windows/amd64`; `make build-gui-linux`; `make package-gui-linux-deb`; `make smoke-android-embedded-host`; `go test ./...`; `go build ./...` | desktop packaging, toolchain pinning, or CI workflow wiring changed |
...
| VPS provider catalog or remote artifact issue service | `docs/vps-provider-catalog-service.md`, `openspec/changes/add-79-vps-provider-catalog-and-artifact-service/specs/vps-provider-catalog-and-artifact-service/spec.md` | `go test ./internal/vpscatalog ./pkg/clientcontrol ./cmd/vps-provider-catalog` | shared client-control types, artifact mapping, compatibility evaluation, or VPS entrypoints changed |
