...
2. `internal/provider/*` resolves provider credentials and probe artifacts.
3. `internal/session` runs the client runtime after provider resolution.
4. `internal/transport` and `internal/tunnelserver` own TURN/DTLS/UDP data-path mechanics.
5. `internal/observe` emits structured events and metrics around runtime stages.
6. `test/compatibility/*` and `test/turnlab` anchor compatibility and integration claims with replayable evidence.

## Subsystem map

| Path | Owns | Read when | Primary checks |
| --- | --- | --- | --- |
| `cmd/probe`, `cmd/tunnel-client`, `cmd/tunnel-server` | CLI flags, dependency wiring, exit/error mapping | Changing flags, startup behavior, stdout/stderr output | `go test ./cmd/...` |
...
| `internal/providerprompt` | interactive provider prompts and browser handoff | Interactive VK/operator flow changes | `go test ./internal/providerprompt` |
