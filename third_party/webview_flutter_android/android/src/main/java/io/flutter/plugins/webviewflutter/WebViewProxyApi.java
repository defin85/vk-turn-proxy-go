...
public class WebViewProxyApi extends PigeonApiWebView {
  ...
  public static class WebViewPlatformView extends WebView implements PlatformView {
    ...
    private int inputConnectionCallCount = 0;
    private int lastImeOptions = 0;
    private int lastInputType = 0;
    private int lastInitialSelectionStart = -1;
    private int lastInitialSelectionEnd = -1;
    @Nullable private String lastPrivateImeOptions = null;
    private boolean lastNoFullscreenFlag = false;
    private boolean lastNoExtractUiFlag = false;
    private boolean lastWindowFocus = false;
    ...
    public InputConnection onCreateInputConnection(@NonNull EditorInfo outAttrs) {
      ...
      inputConnectionCallCount += 1;
      lastImeOptions = outAttrs.imeOptions;
      lastInputType = outAttrs.inputType;
      lastInitialSelectionStart = outAttrs.initialSelStart;
      lastInitialSelectionEnd = outAttrs.initialSelEnd;
      lastPrivateImeOptions = outAttrs.privateImeOptions;
      lastNoFullscreenFlag =
          (outAttrs.imeOptions & EditorInfo.IME_FLAG_NO_FULLSCREEN) != 0;
      lastNoExtractUiFlag =
          (outAttrs.imeOptions & EditorInfo.IME_FLAG_NO_EXTRACT_UI) != 0;
      return connection;
    }

    @Override
    protected void onFocusChanged(
        boolean focused, int direction, @Nullable android.graphics.Rect previouslyFocusedRect) {
    ...
    public Map<String, Object> debugSnapshot() {
      ...
      snapshot.put("input_connection_calls", inputConnectionCallCount);
      snapshot.put("last_ime_options", lastImeOptions);
      snapshot.put("last_ime_options_hex", "0x" + Integer.toHexString(lastImeOptions));
      snapshot.put("last_input_type", lastInputType);
      snapshot.put("last_input_type_hex", "0x" + Integer.toHexString(lastInputType));
      snapshot.put("last_initial_selection_start", lastInitialSelectionStart);
      snapshot.put("last_initial_selection_end", lastInitialSelectionEnd);
      snapshot.put("last_no_fullscreen_flag", lastNoFullscreenFlag);
      snapshot.put("last_no_extract_ui_flag", lastNoExtractUiFlag);
      snapshot.put("last_window_focus", lastWindowFocus);
      snapshot.put("last_focused", lastFocused);
      snapshot.put("webview_has_focus", hasFocus());
      snapshot.put("webview_is_focused", isFocused());
      if (lastPrivateImeOptions != null) {
        snapshot.put("last_private_ime_options", lastPrivateImeOptions);
      }
      final View focusedView = findFocus();
      if (focusedView != null) {
        snapshot.put("webview_find_focus_class", focusedView.getClass().getName());
      }
      return snapshot;
    }
