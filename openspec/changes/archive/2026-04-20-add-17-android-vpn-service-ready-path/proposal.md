...
- Define Android startup as a typed `start -> permission required -> resume` workflow under the canonical control-plane contract instead of a single long-running request or a second Android-only startup API.
