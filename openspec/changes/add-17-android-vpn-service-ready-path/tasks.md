...
- [ ] 1.1 Define the Android-specific `android_vpn_service` capability and startup semantics, including how permission, route validation, host bring-up, runtime attach, and `application_routing_policy` are surfaced through the existing typed control-plane contract
...
- [ ] 2.3 Update the mobile bridge and mobile GUI shell to surface the typed Android ready and failure states plus explicit app-scope selection without inferring support from platform heuristics
