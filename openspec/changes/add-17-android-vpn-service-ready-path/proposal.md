...
- Define supported Android application-routing policies for the first packaged path as explicit host-owned modes such as `all_apps`, `allowed_packages`, or `disallowed_packages`, with fail-closed validation for invalid or mixed package policy.
