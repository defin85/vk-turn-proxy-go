...
- [ ] 1.4 Keep replayable authenticated-contour compatibility evidence separate
...
- [ ] 2.4 Update the mobile owned-browser approval and control-plane
...
- [ ] 2.5 Keep the authenticated hosted-call path valid from a fresh app-owned
      browser session even when remembered embedded sign-in from change `32`
      is unavailable.

## 3. Evidence and Coverage
- [ ] 3.1 Add sanitized compatibility fixtures for the authenticated VK
...
- [ ] 3.1a Keep those authenticated fixtures in
      ...
      naming pattern.
- [ ] 3.1b Update `test/compatibility/vk/fixture.schema.json` and
...
## 4. Validation
- [ ] 4.1 Run `go test ./internal/provider/vk ./internal/androidembeddedhost ./pkg/clientcontrol ./cmd/probe`.
- [ ] 4.2 Run `go test ./test/compatibility/vk/...`.
- [ ] 4.3 Run `cd mobile/gui_shell && flutter analyze && flutter test`.
- [ ] 4.4 Run a live Android Dart MCP owned-browser harness pass from
...
- [ ] 4.5 Run `openspec validate add-37-vk-calls-authenticated-owned-browser-flow --strict --no-interactive`.
