...
build_script_windows_path="$(wslpath -w "${WINDOWS_MIRROR_UNIX}/scripts/build-android-embedded-host.ps1")"
...
sync_back_command="\
\$src='${mirror_windows_path}\\dist\\mobile\\android-embedded-host'; \
\$dst='${source_windows_path}\\dist\\mobile\\android-embedded-host'; \
if (-not (Test-Path \$src)) { throw 'android embedded host artifacts not found in mirror dist' }; \
New-Item -ItemType Directory -Force -Path \$dst | Out-Null; \
...
echo "staged Android embedded host under ${ROOT_DIR}/dist/mobile/android-embedded-host"
