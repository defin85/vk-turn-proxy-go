...
- The verified desktop MCP launch path is: start `go run ./cmd/clientd -listen 127.0.0.1:7777`, then `mcp__dart_desktop__.launch_app(device="linux", root="/home/egor/code/vk-turn-proxy-go/desktop/gui_shell")`.
- After `launch_app` returns a DTD URI, connect `mcp__dart_desktop__` to that URI and use desktop MCP `hot_reload` and inspection tools there.
